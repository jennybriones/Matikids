...
public class MainActivity2Nivel1 extends AppCompatActivity {
    ...
    private ImageView iv_Auno, iv_Ados, iv_chances;
    ...
    public void numAleatorio() {
        if (score <= 9) {
            ...
            } else {            // Si el RESULTADO es mayor a 10...
                numAleatorio(); // ...vuelve a ejecutar el método
            }
        }else{
            Intent intent = new Intent(this, MainActivity2Nivel2.class);
            string_score = String.valueOf(score);
            string_chances = String.valueOf(chances);
            intent.putExtra("jugador", nombre_jugador);  /////  arrastra valores al sgte activity
            intent.putExtra("score", string_score);
            intent.putExtra("chances", string_chances);
    ...
    public void comparar(View view) {
        ...
        if (!respuesta.equals("")) {                             // Si respuesta NO está vacía
            int respuesta_jugador = Integer.parseInt(respuesta);    ///// Se CONVIERTE en entero            /   -  PARSE  -   /
            if (resultado == respuesta_jugador) {                   ///// Si la respuesta es correcta       /  convierte un   /
                mp_great.start();                                   ///// Audio de acertado                 /  tipo de dato   /
                score++;                                            ///// Incrementa score                  /  a otro         /
